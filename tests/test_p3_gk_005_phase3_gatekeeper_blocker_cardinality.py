...
def test_p3_gk_005_phase3_gatekeeper_blocker_set_is_exact_and_bounded() -> None:
    ...
    numbered_blockers = tuple(
        line.strip()
        for line in blockers_section.splitlines()
        if line.strip()[:3] in {"1. ", "2. ", "3. ", "4. ", "5. ", "6. ", "7. ", "8. ", "9. "}
    )

    assert numbered_blockers == EXPECTED_BLOCKERS
    assert len(numbered_blockers) == 3
